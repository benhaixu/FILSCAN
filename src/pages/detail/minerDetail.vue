...
<template>
  <div class="detailApp">
    ...
    <div class="detailHeadTop" @click="gobackNodeDetailFun">
      ...
      <span>{{htmlWordData.minerDetailTitle}}</span>
    </div>
    <div class="detailminer">
      <div class="detailminerHead">
        <span>Miner：{{propData.minerAddress}}</span>
      </div>
      <div class="detailminerFoot">
        <div class="detailItem">
          <span>{{htmlWordData.minerDetailSealedDC}}</span>
          <span class="detailItemValue">{{propData.dataCap}} </span>
        </div>
      </div>
    </div>
    <div class="bannerBox">
      <div class="bannerCom bannerComLeft">
        <div class="headTitle">{{htmlWordData.minerDetailSourceList}}</div>
        <div class="searchCom">
          <Input type="text" :clearable="true" @on-clear="searchFun" :placeholder="htmlWordData.minerDetailInput" suffix="ios-search" style="width:220px;" v-model="searchData.searchKey"
            @keyup.enter.native="searchFun($event)" />
        </div>
        <div class="tableBox">
          <div class="tableHead">
            <div class="itemWidth1 tableItemCom">{{htmlWordData.minerDetailAddress}}</div>
            <div class="itemWidth2 tableItemCom">{{htmlWordData.minerDetailDataCap}}</div>
            <div class="itemWidth3 tableItemCom wordright">{{htmlWordData.minerDetailDealnumbers}}</div>
            <div class="itemWidth3 tableItemCom wordright">{{htmlWordData.minerDetailFailnumbers}}</div>
      ...
      <div class="bannerCom bannerComRight">
        <div class="headTitle">{{htmlWordData.minerDetailDealDetail}}</div>
        <div class="searchCom">
          <DatePicker :value="searchData.timeList" @on-change="searchDetailTimeFun" format="yyyy-MM-dd" type="daterange" split-panels placement="bottom-end" :placeholder="htmlWordData.timeDate" style="width:200px;">
          </DatePicker>
        ...
        <div class="tableBox tableOverFlow">
          <div class="tableHead">
            <div class="detailItemWidth tableItemCom">{{htmlWordData.minerDetailDealProposalCID}}</div>
            <div class="detailItemWidth tableItemCom">{{htmlWordData.minerDetailDealID}}</div>
            <div class="detailItemWidth tableItemCom">{{htmlWordData.minerDetailDealClient}}</div>
            <div class="detailItemWidth tableItemCom">{{htmlWordData.minerDetailDealProvider}}</div>
            <div class="detailItemWidth tableItemCom wordright">{{htmlWordData.minerDetailDealPieceSize}}</div>
            <div class="detailItemWidth tableItemCom wordright">{{htmlWordData.minerDetailDealCreateTime}}</div>
            <div class="detailItemWidth tableItemCom wordright">{{htmlWordData.minerDetailDeaStartTime}}</div>
            <div class="detailItemWidth tableItemCom wordright">{{htmlWordData.minerDetailDeaEndTime}}</div>
            <div class="detailItemWidth tableItemCom wordright">{{htmlWordData.minerDetailDealEpochPrice}}</div>
            <div class="detailItemWidth tableItemCom wordright">{{htmlWordData.status}}</div>
          </div>
          <div v-if="detailData.total>0">
          ...
          <div v-else class="kongClass">
            <img src="../../assets/nodeDetail/illustration／empty@2x.png" alt="" v-show="!detailData.loading">
            <span v-show="!detailData.loading">{{htmlWordData.noData}}</span>
          </div>
        </div>
...
export default {
  ...
  methods: {
    choooseFun(item) {
      ...
      item.select = true;
      this.selectClientAddress = item.client;
    ...
    tablelistFun(page) {
      ...
      params.minerAddress = this.propData.minerAddress;
      if (this.searchData.searchKey) {
        params.client = this.searchData.searchKey
      }
      this.$axios({
        method: 'post',
        url: apis.dataCapMinerClientListApi,
        data: params,
        headers: {
        ...
        if (res.data.code === 0) {
          if (this.tableData.page === 1 && !this.searchData.searchKey) {
            if (localStorage.lanSelctEN === '1') {
              res.data.data.clientMiner.client = "全部"
            } else {
              res.data.data.clientMiner.client = "all"
            }
          ...
          res.data.data.page.records.forEach((element, index) => {
            if (index === 0 && this.tableData.page === 1) {
              element.select = true;
              this.selectClientAddress = element.client;
              this.detialListFun(1)
            } else {
              element.select = false;
            }
    ...
    detialListFun(page) {
      ...
      let params = {};
      params = this.reqiue();
      if (this.selectClientAddress && this.selectClientAddress != '全部' && this.selectClientAddress != 'all') {
        params.client = this.selectClientAddress;
      }
      params.minerAddress = this.propData.minerAddress;
      params.current = page;
      ...
      this.$axios({
        method: 'post',
        url: apis.dataCapMinerClientDetailListApi,
        data: params,
        headers: {
          "Content-Type": "application/json",
          Authorization: token
  ...
  created() {
    ...
    this.propData.minerAddress = this.$route.query.minerAddress;
    this.propData.dataCap = this.$route.query.dataCap;
    this.tablelistFun(1);
  },
}
</script>